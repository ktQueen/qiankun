...
- `main-vue3/src/main.ts`：
  ...
  - `prefetchMicroApps([{ name:'app-vue3', entry:'//localhost:7400' }])`  
    → **只预加载 app3 的 HTML/JS/CSS（只下载，不执行）**。

> 说明：
>
> - **app2 并没有预加载**，而是依赖 `activeRule`：
>   - 当 URL 第一次进入 `/main-vue3/app-vue2/...` 时，qiankun 才去拉取 `//localhost:7200` 的资源并挂载 app2；
> - 这样做的好处是：如果用户从未进入 app2，就不会浪费带宽；
> - app3 因为是“第 3 级嵌套 + 体感开销更大”，才单独做了预加载优化。
...
- 对当前「多技术栈 + 需要嵌套 + 不想重构老项目」的场景，是更合适的选择。
...
有了这套心智模型，以后再遇到类似“主应用 + 多级嵌套子应用”的需求，可以很快地拎清楚：
